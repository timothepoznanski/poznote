...
$action = $_POST['action'] ?? '';
// Optional workspace parameter to scope folder operations
...
switch($action) {
    ...
    $defaultFolderName = getDefaultFolderName($workspace);
        ...
        // Check if folder already exists in entries or folders table (optionally scoped to workspace)
        if ($workspace !== null) {
    ...
    case 'rename':
        ...
        // Ensure target name does not already exist in the same workspace (allow same name in other workspaces)
        if ($workspace !== null) {
    ...
    if ($stmt1->execute([$newName, $oldName]) && ($workspace !== null ? $stmt2->execute([$newName, $oldName, $workspace, $workspace]) : $stmt2->execute([$newName, $oldName]))) {
            echo json_encode(['success' => true]);
        } else {
            echo json_encode(['success' => false, 'error' => 'Database error']);
        }
    ...
    case 'empty_folder':
        ...
        if ($workspace !== null) {
            $query .= " AND (workspace = :ws OR (workspace IS NULL AND :ws = 'Poznote'))";
            $stmt = $con->prepare($query);
            $stmt->bindValue(':ws', $workspace);
            $execSuccess = $stmt->execute([$folderName]);
        } else {
            $stmt = $con->prepare($query);
            $execSuccess = $stmt->execute([$folderName]);
        }
