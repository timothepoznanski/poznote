...
<details>
<summary><strong>Утерян пароль администратора</strong></summary>
<br>

Если другой администратор ещё может войти, он может задать вам новый пароль в **Settings > Admin Tools > User Management**. Иначе сбросьте его прямо в главной базе данных. Из каталога Poznote, на хосте (утилита командной строки `sqlite3` должна быть установлена там, в образ Poznote она не входит):

```bash
sudo sqlite3 data/master.db "UPDATE users SET password_hash=NULL, password_login_disabled=0 WHERE id=1;"
```

Это стирает сохранённый пароль первой учётной записи (id 1, всегда администратор), и снова действует встроенное значение по умолчанию: войдите с именем пользователя этой учётной записи и паролем `admin`, затем сразу смените его в **Settings > Change Password**. Чтобы сбросить другую учётную запись, замените `WHERE id=1` на `WHERE username='её_имя'`; для обычного пользователя действует пароль `user`.

Если для этой учётной записи включена двухфакторная аутентификация, форма входа по-прежнему запрашивает код после пароля. См. следующий пункт, если утеряно и устройство.

</details>

<a id="two-factor-lockout"></a>
<details>
<summary><strong>Вход заблокирован двухфакторной аутентификацией (утеряны устройство и коды восстановления)</strong></summary>
<br>

Каждый код восстановления, выданный при включении двухфакторной аутентификации, позволяет войти один раз: на экране кода выберите **Use a recovery code**. Если кодов не осталось, администратор может выключить для вас двухфакторную аутентификацию в **Settings > Admin Tools > User Management**, в диалоге пароля вашей учётной записи.

Если вы единственный администратор, удалите второй фактор прямо в главной базе данных. Из каталога Poznote, на хосте (утилита командной строки `sqlite3` должна быть установлена там):

```bash
sudo sqlite3 data/master.db "DELETE FROM user_totp_recovery_codes WHERE user_id=1; DELETE FROM user_totp WHERE user_id=1;"
```

Замените `1` на id учётной записи (`sudo sqlite3 data/master.db "SELECT id, username FROM users;"` выводит их список). Для входа снова достаточно одного пароля, cookie «Remember me», выданные для этой учётной записи, перестают работать, а двухфакторную аутентификацию можно настроить заново в **Settings > Two-factor authentication**.
