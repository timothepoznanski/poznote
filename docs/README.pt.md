...
Não quer administrar um servidor? Um provedor de hospedagem pode rodar o Poznote para você e mantê-lo online. Veja os provedores disponíveis e como escolher entre eles [aqui](https://poznote.com/hosting.html).
...
Muitas vezes o `sudo` não é necessário aqui: se o seu usuário já tem o uid `1000`, o `chown` pode ser pulado, e no Podman/Docker rootless ele pode ser executado sem root, veja [Execução rootless](TROUBLESHOOTING.pt.md#running-rootless).
...
Para migrar uma instância existente do Poznote para a variante rootless, ou para mais detalhes, veja [Execução rootless](TROUBLESHOOTING.pt.md#running-rootless) no Guia de solução de problemas.

</details>

<br>

> Se encontrar problemas na instalação, consulte o [Guia de solução de problemas](TROUBLESHOOTING.pt.md).
...
- `POZNOTE_PHP_FPM_MAX_CHILDREN` para alterar o número de requisições PHP simultâneas (padrão 10) em uma instância muito usada, veja o [Guia de solução de problemas](TROUBLESHOOTING.pt.md#the-app-stops-answering-under-load)
- `POZNOTE_PHP_MEMORY_LIMIT` para alterar o limite de memória do PHP por requisição, em MB (padrão 512), veja o [Guia de solução de problemas](TROUBLESHOOTING.pt.md#a-request-runs-out-of-memory)
- `POZNOTE_LISTEN_PORT` para alterar a porta em que o servidor web escuta dentro do contêiner (padrão 80), necessário apenas com `network_mode: host`, veja o [Guia de solução de problemas](TROUBLESHOOTING.pt.md#running-with-host-network)
...
Cada usuário pode ativar a autenticação de dois fatores (TOTP) em **Settings > Two-factor authentication**. Depois da senha, o formulário de login passa a pedir um código de 6 dígitos de um aplicativo autenticador (Aegis, Google Authenticator, 1Password, Bitwarden...).

- A configuração mostra um QR code, desenhado no navegador, e dez códigos de recuperação de uso único, para guardar caso o celular seja perdido.
- Ela protege o login com senha. Um login por SSO fica a cargo do provedor de identidade e do próprio segundo fator dele.
- Enquanto estiver ativada, a API REST deixa de aceitar só a senha da conta: dê aos clientes uma [senha de aplicativo](#senhas-de-aplicativo), ou envie o código atual no cabeçalho `X-Poznote-OTP`.
- Um administrador pode desativá-la para um usuário que perdeu o aparelho e os códigos de recuperação, em **Settings > Admin Tools > User Management** (janela da senha).
- Ativá-la ou desativá-la invalida os cookies "Remember me" existentes desse usuário.
...
9. A [autenticação de dois fatores](#autenticação-de-dois-fatores) cobre apenas o formulário de senha: um login por SSO nunca pede o código do Poznote. Exija um segundo fator no provedor de identidade. Um usuário que tem senha e identidade SSO mantém os dois acessos, e o acesso por SSO só é tão forte quanto a política do provedor.
...
A lista completa de limites e os endpoints que gerenciam as senhas de aplicativo estão na [documentação da API REST](API-REST.md#authentication).
...
- **Compartilhamento**: notas e pastas podem ser compartilhadas com outros usuários da instância, somente leitura ou editáveis, ou publicamente por links dedicados. Um espaço de trabalho inteiro pode ser compartilhado com outros usuários da instância, que o encontram no menu de espaços de trabalho e o editam junto com o proprietário. Quando vários usuários têm acesso à mesma nota, apenas um a edita por vez e os demais veem quem detém o bloqueio.
- **Editar a mesma nota**: só uma pessoa edita de cada vez. Quando uma nota está bloqueada, o banner de somente leitura oferece **assumir a edição**: a tela da outra pessoa passa a somente leitura e as alterações que ela não salvou ficam no navegador dela, oferecidas de novo assim que a nota estiver livre. Uma nota aberta recebe em poucos segundos as alterações feitas em outro lugar. Em uma nota **Markdown** com alterações não salvas dos dois lados, os dois conjuntos de alterações são mesclados automaticamente quando tocam linhas diferentes, e um banner deixa você escolher quando se sobrepõem. Notas de texto formatado nunca são mescladas, você escolhe qual versão manter. Mantenha scripts e outros códigos em blocos de código delimitados (```` ``` ````): HTML bruto fora de um bloco de código é sanitizado ao salvar, o que pode fazer uma mesclagem limpa parecer um conflito.
...
As entregas são assinadas com HMAC-SHA256 quando o webhook tem um segredo, e o conteúdo das notas nunca é enviado. Todos os eventos, os campos do payload, a verificação da assinatura e as garantias de entrega estão descritos na **[documentação de Webhooks](WEBHOOKS.pt.md)**.
...
O Poznote continua funcionando sem rede de duas formas: as notas que você modificou recentemente, seus favoritos e as notas ou pastas que você escolher manter continuam disponíveis no seu navegador, prontas para serem lidas e editadas, e um backup completo pode ser consultado em qualquer lugar como uma exportação somente leitura.
...
O que o assistente sabe fazer, a escolha de um provedor e de um modelo, as chaves de API pessoais e a conexão de um servidor local a partir do contêiner do Poznote estão descritos na [documentação do Assistente IA](AI-ASSISTANT.pt.md). Para deixar um assistente de IA externo (VS Code Copilot, Claude CLI...) gerenciar suas notas, veja o [Servidor MCP](#servidor-mcp) abaixo.
...
A configuração de um servidor, a escolha do modelo e todo o resto estão na [documentação da Transcrição](TRANSCRIPTION.pt.md).
...
A extensão se conecta à sua instância com o seu nome de usuário e uma [senha de aplicativo](#senhas-de-aplicativo). Os passos de configuração estão na [documentação da extensão do Chrome](CHROME-EXTENSION.pt.md).
...
Para a referência completa da API, com todos os endpoints, parâmetros e exemplos com curl, consulte a **[Documentação da API REST](API-REST.md)**.
