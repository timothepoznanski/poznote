...
测试版（beta）会在稳定版发布之前带来新功能，并在[发布页面](https://github.com/timothepoznanski/poznote/releases)上标记为预发布版本。测试版以 `latest-and-beta` 标签发布，该标签始终指向最新版本，无论是测试版还是稳定版。

要使用测试版，请修改 `docker-compose.yml` 中的两行 `image`，文件的其余部分保持不变：
```yaml
services:
  webserver:
...
使用 [rootless](#rootless) 变体时，`docker-compose.rootless.yml` 中的 webserver 镜像改为 `poznote:latest-and-beta-rootless`，MCP 镜像保持不变，同样为 `poznote-mcp:latest-and-beta`。
...
<details>
<summary><strong>富文本笔记</strong></summary>
&nbsp;
...
*   **插入到笔记中：** 在富文本或 Markdown 笔记中输入 `/template`（或 `/` 加模板标题），然后选择一个模板：其内容会粘贴到光标处，如果模板与笔记类型不同，还会自动转换。
...
*   **格式：** 新条目会创建为富文本或 Markdown 笔记，取决于 **设置 > 行为** 中的“日记条目格式”设置。
</details>
...
*   **自动：** 每天首次打开笔记时会创建一个快照。每条笔记保留最近 3 个自动快照，这个数量可以在 **设置 > 操作 > 快照** 中修改。
...
- **日记条目格式：** 将日记条目创建为富文本或 Markdown 笔记
...
- **共享**：笔记和文件夹可以与实例上的其他用户共享（只读或可编辑），也可以通过专用链接公开共享。整个工作区可以与实例上的其他用户共享，他们会在自己的工作区菜单中看到它，并与所有者一起编辑。当多个用户可以访问同一条笔记时，同一时间只有一人编辑，其他人可以看到当前是谁持有锁。
- **编辑同一条笔记**：同一时间只有一人编辑。笔记被锁定时，只读横幅会提供**接管**选项：对方的界面变为只读，其未保存的更改会保留在对方浏览器中，笔记空闲后再次提供。已打开的笔记会在几秒内获取他处所做的更改。对于双方都有未保存更改的 **Markdown** 笔记，若两组更改涉及不同的行，会自动合并；若相互重叠，横幅会让您选择。富文本笔记从不合并，由您选择保留哪个版本。请将脚本和其他代码放在围栏代码块（```` ``` ````）中：代码块之外的原始 HTML 会在保存时被清理，可能让本可干净合并的更改看起来像冲突。
...
使用笔记工具栏中的 **导出** 按钮导出单条笔记：
  ...
  - **富文本笔记：** 导出为 HTML，或导出为嵌入图片的单个 HTML 文件
...
Poznote 在没有网络时也能以两种方式继续使用：最近修改的笔记、您的收藏以及您选择保留的笔记或文件夹会保留在您的浏览器中，可以阅读和编辑；完整备份则可以作为只读导出在任何地方浏览。

<details>
<summary><strong>离线笔记</strong></summary>
<br>

最近 5 天内修改的笔记会保存在您使用 Poznote 的每个浏览器中，并且每次保存后都会更新这份副本。在教室、火车上或任何没有 Wi-Fi 的地方，打开平常使用的 Poznote 地址即可：浏览器会显示 Poznote 的离线版本，侧边栏、编辑器、工具栏、搜索和标签页都与平时相同（双击或中键单击可在新标签页中打开笔记，在线时打开的标签页也会保留），但只列出离线保存的笔记。

*   **登录：** 输入您上次在此浏览器登录时使用的密码，验证无需服务器。如果您从未在此浏览器中输入过密码（SSO、自动登录），上次使用的账户可通过 **以（账户名）身份继续** 按钮直接打开。
*   **阅读和编辑：** 富文本笔记、Markdown 笔记和任务列表会在平常的编辑器中打开，也可以新建笔记。/ 菜单和右键菜单也可以使用，但不包括需要服务器的命令（上传图片和文件、模板、绘图、指向其他笔记的链接）。其他类型的笔记（例如绘图）仅在线可用。修改会保存在浏览器中，直到发送出去。
*   **离线保留：** 收藏始终会保留；在笔记或文件夹（含子文件夹）的菜单中选择 **离线保留**，无论日期如何都会保留，并包含全部附件（PDF、音频、文件），每个最多 25 MB。笔记的同一菜单会显示它在此浏览器中是否可离线使用，“笔记”和“文件夹”页面也会标出在此浏览器中可离线使用的笔记和文件夹。“笔记”页面的批量操作还可以一次离线保留多条笔记（或取消），“文件夹”页面每个文件夹的菜单中也有 **离线保留**。
*   **离线页面：** 图标侧栏中的 **离线** 按钮会像“共享”页面一样列出离线保留的笔记和文件夹：文件夹以树形显示并包含其笔记，显示每条笔记被保留的原因（离线保留、位于离线保留的文件夹中、收藏、最近修改），提供筛选，以及用于离线保留或停止保留笔记的按钮。警告图标表示此浏览器尚未保存该笔记。
*   **恢复联网：** 修改会自动发送。如果同一笔记在此期间也在服务器上被修改，会尽可能合并两个版本；否则您的离线版本会另存为一条名为“...（离线副本）”的笔记。在此期间于服务器上被删除的笔记会被重新创建。
*   **设置：** **设置 > 操作 > 离线笔记** 用于设置保留多少天内的笔记（默认 5 天，最多 30 天，0 表示关闭离线笔记），并显示当前浏览器中保存的内容。
*   **限制：** 最多 300 条笔记和 50 MB 文本，优先保留最近修改的。文件也会保存：这些笔记中显示的图片，以及收藏和 **离线保留** 笔记的全部附件（PDF、音频、文件），每个最多 25 MB，总计 400 个、200 MB，且绝不超过浏览器剩余空间的一半。更大的文件只能在线使用，离线打开笔记时会有提示。
*   **要求：** Poznote 必须通过 HTTPS 提供（浏览器只会在安全连接下离线保存页面，`http://localhost` 也可以），并且登录后至少在该浏览器中在线打开过一次，才会创建副本。
*   **隐私：** 只保存您自己账户的笔记，不包括与您共享的账户或工作区中的笔记。它们以未加密的形式存储在浏览器中。退出登录会将它们从浏览器中删除（包括尚未发送的修改，删除前会有警告列出它们）：在共用电脑上，离开时请退出登录。没有网络时也可以在离线页面退出登录：笔记会立即删除，服务器上的会话会在下次联网打开 Poznote 时结束。

</details>

<details>
<summary><strong>离线导出</strong></summary>
<br>
