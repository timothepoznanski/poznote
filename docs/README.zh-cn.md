...
Poznote 在没有网络时也能以两种方式继续使用：最近修改的笔记会保留在您的浏览器中，可以阅读和编辑；完整备份则可以作为只读导出在任何地方浏览。

<details>
<summary><strong>离线笔记</strong></summary>
<br>

最近 5 天内修改的笔记会保存在您使用 Poznote 的每个浏览器中，并且每次保存后都会更新这份副本。在教室、火车上或任何没有 Wi-Fi 的地方，打开平常使用的 Poznote 地址即可：浏览器会显示 Poznote 的离线版本，侧边栏、编辑器、工具栏和搜索都与平时相同，但只列出离线保存的笔记。

*   **登录：** 输入您上次在此浏览器登录时使用的密码，验证无需服务器。如果您从未在此浏览器中输入过密码（SSO、自动登录），上次使用的账户可通过 **以（账户名）身份继续** 按钮直接打开。
*   **阅读和编辑：** HTML 笔记、Markdown 笔记和任务列表会在平常的编辑器中打开，也可以新建笔记。其他类型的笔记（例如绘图）仅在线可用。修改会保存在浏览器中，直到发送出去。
...
*   **限制：** 最多 300 条笔记和 50 MB 文本，优先保留最近修改的。这些笔记中显示的图片也会保存，每张最多 8 MB，总计 400 张、200 MB，且绝不超过浏览器剩余空间的一半。其他附件（PDF、音频、文件）只能在线打开。
